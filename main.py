...
async def api_me(request):
    ...
    return web.json_response(
        {"user": telegram_user, "profile": user, "ref_count": ref_count, "ref_earned": ref_earned}
    )
...
async def api_case_access(request):
    ...
    return web.json_response(
        {"ok": subscribed and available, "subscribed": subscribed, "available": available, "channel": REQUIRED_CASE_CHANNEL}
    )
...
async def api_free_case(request):
    ...
    if user["shared_count"] < 2:
        return web.json_response(
            {"ok": False, "message": f"Поделитесь ссылкой 2 раза. Прогресс: {user['shared_count']}/2", "need_share": True}
        )
...
async def api_ton_deposit(request):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            ...
            (uid, intent_hash, amount, BOT_WALLET_ADDRESS, "pending", int(time.time())),
        )
        await db.commit()

    uri = f"ton://transfer/{BOT_WALLET_ADDRESS}?amount={int(amount * 1e9)}&text={quote(comment)}"
    return web.json_response(
        {"ok": True, "amount": amount, "comment": comment, "destination": BOT_WALLET_ADDRESS, "ton_uri": uri}
    )
...
async def api_ton_confirm(request):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            "SELECT deposit_id,amount_ton FROM ton_deposits WHERE user_id=? AND status='pending' ORDER BY deposit_id DESC LIMIT 1",
            (uid,),
        ) as cursor:
    ...
    if isinstance(destination_data, dict):
        destination = str(destination_data.get("address") or "")
    else:
        destination = str(destination_data or "")
    ...
    if (
        (destination and destination != BOT_WALLET_ADDRESS)
        or value < int(expected * 1e9)
        or not text.startswith(f"GIFTSMMS:{uid}:")
    ):
        return web.json_response({"ok": False, "message": "Сумма, адрес или комментарий не совпадают с заявкой."})

    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "UPDATE ton_deposits SET tx_hash=?,status='confirmed',confirmed_at=? WHERE deposit_id=? AND status='pending'",
            (tx_hash, int(time.time()), intent[0]),
        )
        await db.commit()
...
async def main():
    ...
    app.router.add_get("/battle-virtual.js", file_handler("battle-virtual.js"))
    app.router.add_get("/ton-payments.js", file_handler("ton-payments.js"))
    app.router.add_get("/animations.css", file_handler("animations.css"))
    app.router.add_get("/giftsmms-logo.svg", file_handler("giftsmms-logo.svg"))

    async def ton_manifest(request):
        base = str(request.url.with_path("/").with_query(""))
        return web.json_response(
            {"url": base, "name": APP_NAME, "iconUrl": f"{base}giftsmms-logo.svg"}
        )
