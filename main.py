...
async def api_ton_deposit(request):
    ...
    try: amount=float((await request.json()).get("amount",0))
...
async def api_ton_confirm(request):
    ...
    try: tx_hash=str((await request.json()).get("tx_hash","")).strip().lower()
    ...
    try:
        async with ClientSession() as session:
            async with session.get(f"https://tonapi.io/v2/blockchain/transactions/{tx_hash}",headers={"Authorization":f"Bearer {TON_API_KEY}"},timeout=12) as r:
