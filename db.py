...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.execute("""CREATE TABLE IF NOT EXISTS battle_players (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            battle_id INTEGER,
            user_id INTEGER,
...
async def get_user(user_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT balance, ref_count, ref_earned, free_case_time, ton_address, shared_count, last_share_reset FROM users WHERE user_id=?", (user_id,)) as cursor:
...
async def update_balance(user_id: int, amount: float, commission: float = 0.0):
    if amount > 0 and commission > 0:
        amount = int(amount * (1 - commission))
    async with aiosqlite.connect(DB_NAME) as db:
...
async def add_referral(inviter_id: int, invited_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        if cursor.rowcount > 0:
            await update_balance(inviter_id, 1, commission=0.0)
            await db.execute("UPDATE users SET ref_count = ref_count + 1, ref_earned = ref_earned + 1 WHERE user_id=?", (inviter_id,))
            await db.commit()
...
async def create_battle(creator_id: int, currency: str, max_players: int = 10):
    hash_str = "".join(random.choices(string.hexdigits, k=8)) + "..." + "".join(random.choices(string.hexdigits, k=4))
    async with aiosqlite.connect(DB_NAME) as db:
        cursor = await db.execute(
            """INSERT INTO battles (creator_id, currency, max_players, hash, created_at)
               VALUES (?, ?, ?, ?, ?)""",
            (creator_id, currency, max_players, hash_str, int(time.time())),
        )
        battle_id = cursor.lastrowid
        await db.commit()
        return battle_id

async def add_player_to_battle(battle_id: int, user_id: int, bet_stars: int = 0, bet_ton: float = 0.0, ton_address: str = None):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT max_players, total_bank_stars, total_bank_ton, status FROM battles WHERE battle_id=?", (battle_id,)) as cursor:
            row = await cursor.fetchone()
            if not row or row[3] != "waiting":
                return False, "Игра не найдена или уже началась"
            max_players, bank_stars, bank_ton = row[0], row[1], row[2]
        ...
        async with db.execute("SELECT * FROM battle_players WHERE battle_id=? AND user_id=?", (battle_id, user_id)) as cursor:
            if await cursor.fetchone():
                return False, "Вы уже участвуете"

        await db.execute(
            """INSERT INTO battle_players (battle_id, user_id, bet_stars, bet_ton, ton_address, joined_at)
               VALUES (?, ?, ?, ?, ?, ?)""",
...
async def get_battle_players(battle_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT user_id, bet_stars, bet_ton, ton_address FROM battle_players WHERE battle_id=?", (battle_id,)) as cursor:
...
async def get_active_battles():
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute('SELECT battle_id, creator_id, currency, total_bank_stars, total_bank_ton, hash, max_players FROM battles WHERE status="waiting"') as cursor:
            return await cursor.fetchall()
