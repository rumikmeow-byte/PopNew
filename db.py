...
async def set_free_case_time(user_id: int, timestamp: int):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "UPDATE users SET free_case_time = ? WHERE user_id = ?",
            (timestamp, user_id),
        )
        await db.commit()
...
async def reset_share_count(user_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            """
            UPDATE users
            SET shared_count = 0, last_share_reset = ?
            WHERE user_id = ?
            """,
...
async def increment_share_count(user_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "UPDATE users SET shared_count = shared_count + 1 WHERE user_id = ?",
            (user_id,),
        )
        await db.commit()
...
async def get_referral_stats(user_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute(
            "SELECT ref_earned FROM users WHERE user_id = ?",
            (user_id,),
        ) as cursor:
            row = await cursor.fetchone()
...
async def get_all_channels():
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            "SELECT username FROM channels ORDER BY id"
        ) as cursor:
...
async def create_battle(creator_id: int, currency: str, max_players: int = 10):
    ...
    hash_str = (
        ...
        + "..."
        + "".join(random.choices(string.hexdigits, k=4))
    )
...
async def add_player_to_battle(
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            """
            SELECT max_players, total_bank_stars, total_bank_ton, status
        ...
        await db.execute(
            """
            UPDATE battles
            SET total_bank_stars = total_bank_stars + ?,
                total_bank_ton = total_bank_ton + ?
            WHERE battle_id = ?
            """,
...
async def get_battle_players(battle_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            """
            SELECT user_id, bet_stars, bet_ton, ton_address
            FROM battle_players WHERE battle_id = ? ORDER BY id
            """,
            (battle_id,),
        ) as cursor:
...
async def get_active_battles():
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            """
            SELECT battle_id, creator_id, currency,
                   total_bank_stars, total_bank_ton, hash, max_players
            FROM battles
