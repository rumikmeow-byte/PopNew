...
async def init_db_tables(db_pool):
    """Инициализация таблиц с поддержкой звезд и рефералов"""
...
# ==================== 2. СТАРТ И РЕФЕРАЛЬНАЯ СИСТЕМА (0.85 ⭐) ====================
...
async def start_cmd(message: types.Message, db_pool):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if not user_exists:
                ...
                # Начисляем 0.85 звезд пригласителю
                if referrer_id:
                    await conn.execute("""
                        UPDATE users 
                    ...
                    try:
                        await message.bot.send_message(
                            referrer_id, 
                            f"🎉 По вашей ссылке зарегистрировался новый игрок! Вам начислено **0.85 ⭐**"
                        )
                    except Exception:
                        pass
    ...
    kb = types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="👥 Рефералы & Баланс", callback_data="ref_system")],
        ...
        [types.InlineKeyboardButton(text="💣 Игра «Мины»", callback_data="start_mines"), types.InlineKeyboardButton(text="🎁 0.1 TON Бонус", callback_data="daily_free_spin")]
    ])
...
async def show_ref_info(callback: types.CallbackQuery, db_pool):
    """Просмотр баланса звёзд и рефералов"""
    async with db_pool.acquire() as conn:
