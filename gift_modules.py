...
# Берем список ADMIN_IDS из Render (формат в Render: 123456789,987654321)
raw_admins = os.getenv("ADMIN_IDS", "")
ADMIN_IDS = [int(admin_id.strip()) for admin_id in raw_admins.split(",") if admin_id.strip().isdigit()]

# Адрес кошелька из Render
BOT_TON_WALLET = os.getenv("BOT_TON_WALLET", "EQ_УКАЖИТЕ_КОШЕЛЕК_В_RENDER")
...
async def init_db_tables(db_pool):
    """Создание таблиц в PostgreSQL / SQLite"""
...
async def start_cmd(message: types.Message, db_pool):
    """Обработка /start и начисление 0.85 звезд пригласившему"""
                ...
                if referrer_id:
                    ...
                    try:
                        await message.bot.send_message(
                            referrer_id, 
                            "🎉 По вашей ссылке зарегистрировался новый игрок! Вам начислено **0.85 ⭐**",
                            parse_mode="Markdown"
                        )
                    except Exception:
                        pass
...
async def show_deposit_info(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"В поле «Комментарий» (Memo) обязательно укажите:\n"
        f"`dep_{user_id}`\n\n"
...
async def process_withdraw_request(message: types.Message, db_pool):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                "INSERT INTO transactions (user_id, type, amount, wallet_address, status) VALUES ($1, 'withdraw', $2, $3, 'pending')",
